...
async function loadBoard(slug){
  ...
  // fill payouts + lock
  els.payouts.q1.value = board.payouts?.q1 ?? 5;
  els.payouts.ht.value = board.payouts?.ht ?? 15;
  els.payouts.q4.value = board.payouts?.q4 ?? 5;
  els.payouts.final.value = board.payouts?.final ?? 25;
...
els.randomizeBtn.addEventListener("click", async ()=>{
  const token = els.token.value.trim(); if (!token) return alert("Enter admin password.");
  // ✅ Confirmation modal
  if (!confirm("Randomize header numbers now? This can be done only once and cannot be undone.")) return;
...
els.exportBtn.addEventListener("click", async ()=>{
  ...
  (data||[]).forEach(r=> rows.push([r.square_idx+1, r.buyer_name||"", r.email||"", r.status, r.paid_at||"", r.created_at||""]));
...
els.cb.createBtn.addEventListener("click", async ()=>{
  ...
  const payload = {
    ...
    p_payouts: { q1:5, ht:15, q4:5, final:25 }, // defaults; you can change below and Save Settings
    p_is_open: true,
    p_admin_token: p.pwd
  };
  ...
  els.slug.value = p.slug; els.token.value = p.pwd;
...
els.payouts.saveBtn.addEventListener("click", async ()=>{
  ...
  const lockISO = els.payouts.lockAt.value
    ? new Date(els.payouts.lockAt.value).toISOString()
    : null;
