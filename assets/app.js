...
function Setup({ root, board, setBoard }){
  ...
  const exportCSV = () => {
    ...
    const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(new Blob([csv],{type:"text/csv"})), download: `${(board.boardTitle||"roster").replace(/\s+/g,"_")}.csv` });
  ...
  return html`
    ...
    <div class="row">
      ...
      <div>
        <label class="label">New Board</label>
        <div style="display:flex;gap:8px">
          <input id="newBoardName" placeholder="e.g., Week 3 vs. Hornets"/>
          <button class="primary" onClick=${()=>{ const el=document.getElementById("newBoardName"); if(el.value) root.createBoard(el.value); }}>Create</button>
        </div>
      </div>
      <div>
    ...
    <div class="row">
      <div>
        ...
        <input value=${board.boardTitle} onInput=${e=>set("boardTitle", e.target.value)} ${VIEW_ONLY?"disabled":""}/>
      </div>
      <div>
        <label class="label">Venmo Handle</label>
        <input value=${board.venmoHandle} onInput=${e=>set("venmoHandle", e.target.value)} ${VIEW_ONLY?"disabled":""}/>
      </div>
      <div>
        <label class="label">Top Team</label>
        <input value=${board.teamTop} onInput=${e=>set("teamTop", e.target.value)} ${VIEW_ONLY?"disabled":""}/>
      </div>
      <div>
        <label class="label">Side Team</label>
        <input value=${board.teamSide} onInput=${e=>set("teamSide", e.target.value)} ${VIEW_ONLY?"disabled":""}/>
      </div>
      <div>
        <label class="label">Cost per Square ($)</label>
        <input type="number" min="0" value=${board.costPerSquare} onInput=${e=>set("costPerSquare", parseFloat(e.target.value)||0)} ${VIEW_ONLY?"disabled":""}/>
      </div>
      <div>
        <label class="label">Theme Color</label>
        <input type="color" value=${board.themeColor} onInput=${e=>{ document.documentElement.style.setProperty("--brand", e.target.value); set("themeColor", e.target.value); }} ${VIEW_ONLY?"disabled":""}/>
      </div>
    </div>

    <div class="controls" style="margin-top:8px">
      <button class="primary" onClick=${()=>{ if(!board.numbersLocked && !VIEW_ONLY) setBoard({...board, topNums:shuffle10(), sideNums:shuffle10()}); }} ${VIEW_ONLY?"disabled":""}>Randomize</button>
      <button onClick=${()=>set("numbersLocked", !board.numbersLocked)} ${VIEW_ONLY?"disabled":""}>${board.numbersLocked?"Unlock":"Lock"}</button>
...
function Board({ board, setBoard }){
  ...
  const winners = useMemo(()=> ({
    q1: win("q1"), q2: win("q2"), q3: win("q3"), q4: win("q4")
  ...
  return html`
    <div>
      ...
      <div class="board-wrap">
        <div class="board">
          <div></div>
          ${board.topNums.map(n=>html`<div class="cell header sticky-top" style="text-align:center;font-weight:600">${board.teamTop} ${n}</div>`)}
...
/* ---------- App mounting: render into existing placeholders ---------- */
function App(){
  const root = useRoot();
  ...
  useEffect(()=>{
    try{
      ...
      if(rootRaw){
        ...
        const bRaw = activeId ? localStorage.getItem(BOARD_PREFIX + activeId) : null;
        if(bRaw){
  ...
  // Render into #setup and #board so we don't replace the header
  useEffect(()=>{
  ...
  return null; // nothing to mount directly
}

render(html`<${App}/>`, document.createElement("div")); // mount a tiny controller
